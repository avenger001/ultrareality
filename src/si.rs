...
impl Si {
    ...
    pub fn write(&mut self, paddr: u32, value: u32, rdram: &mut PhysicalMemory, pif: &mut Pif) {
        let Some(off) = Self::offset(paddr) else {
            return;
        };
        match off {
            ...
            0x04 => {
                self.pif_addr = value;
                self.dma_rd64(rdram, pif);
            }
            0x10 => {
                self.pif_addr = value;
                self.dma_wr64(rdram, pif);
            }
            _ => {}
        }
    }
    ...
    fn dma_rd64(&mut self, rdram: &mut PhysicalMemory, pif: &Pif) {
        self.status |= 3;
    ...
    fn dma_wr64(&mut self, rdram: &mut PhysicalMemory, pif: &mut Pif) {
        self.status |= 3;
        let base = pif_ram_byte_index(self.pif_addr);
        let dram = (self.dram_addr & 0x00FF_FFFF) as usize;
        let rdram_len = rdram.data.len();
...
mod tests {
    ...
    fn si_dma_round_trip_pif_ram() {
        ...
        si.write(SI_REGS_BASE, 0x0010_0000, &mut rdram, &mut pif);
        si.write(SI_REGS_BASE + 0x04, PIF_RAM_START, &mut rdram, &mut pif);

        pif.ram[0..4].fill(0);
        // RDRAM → PIF RAM (SI write DMA)
        si.write(SI_REGS_BASE + 0x10, PIF_RAM_START, &mut rdram, &mut pif);
